...
#ifndef NETIFD_WITH_BCM_IPTV	
	/*when ipv6-pass-through module loaded, br-wan will be held*/
	if(!strcmp(bridge->ifname, "br-wan")){
		snprintf(cmdStr, sizeof(cmdStr), "[ -n \"$(lsmod | grep ipv6_pass_through)\" ] && rmmod ipv6_pass_through");
		system(cmdStr);
	}

...
ovs_bcm_blog_mega_get_key_t ovs_bcm_blog_mega_get_key_fn = NULL;
ovs_bcm_blog_mega_set_key_t ovs_bcm_blog_mega_set_key_fn = NULL;
#endif
...
#if defined(CONFIG_BCM_OVS)
ovs_bcm_blog_mega_put_fast_stats_t ovs_bcm_blog_mega_put_fast_stats_fn = NULL;
void ovs_bcm_blog_mega_put_fast_stats(void *net_p, blog_fast_stats_t *stats)
{
    if (ovs_bcm_blog_mega_put_fast_stats_fn)
        ovs_bcm_blog_mega_put_fast_stats_fn(net_p, stats);
}
#endif
                       ...
                       unsigned long param1, unsigned long param2 )
{
    ...
    switch ( request )
    {
        ...
        case MEGA_KEY_SET:
        {
            if (ovs_bcm_blog_mega_set_key_fn)
                ovs_bcm_blog_mega_set_key_fn(net_p, param2);
            return 0;
        }

        case MEGA_KEY_GET:
        {
            if (ovs_bcm_blog_mega_get_key_fn)
                ret = ovs_bcm_blog_mega_get_key_fn(net_p);
            else
                ret = BLOG_KEY_FC_INVALID;
            break;
        }
        ...
        case MEGA_PUT_STATS:
        {
            ...
            if (IS_BLOG_NPE_MEGA(blog_p))
            {
                ...
                {
                    if (npe_p->nwe_p)
                        ovs_bcm_blog_mega_put_fast_stats(npe_p->nwe_p, stats_p);
                }
            }
            ret = 0;
        }
...
void blog_ct_dump(Blog_t *blog_p)
{
    ...
    if (nwe_pld_p)
        blog_nfct_dump(blog_p->skb_p, nwe_pld_p, blog_p->nf_dir_pld);

    if (nwe_del_p)
        blog_nfct_dump(blog_p->skb_p, nwe_del_p, blog_p->nf_dir_del);
#endif
}
...
void blog_mega_dump(Blog_t *blog_p)
{
    ...
    printk("\tMEGA: mega_p<0x%p>, key[0x%08x]\n",
            nwe_mega_p, nwe_mega_p ? (unsigned int) ovs_bcm_blog_mega_get_key_fn(nwe_mega_p): 0);
#endif
}
...
#if defined(CONFIG_BCM_OVS)
EXPORT_SYMBOL(ovs_bcm_blog_mega_put_fast_stats_fn);
#endif
...
#if defined(CONFIG_BCM_OVS)
EXPORT_SYMBOL(ovs_bcm_blog_mega_get_key_fn);
EXPORT_SYMBOL(ovs_bcm_blog_mega_set_key_fn);
#endif

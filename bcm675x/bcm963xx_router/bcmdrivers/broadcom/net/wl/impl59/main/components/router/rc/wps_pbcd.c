 ...
 *
 * $Id: wps_pbcd.c 775124 2019-05-20 20:09:06Z $
 */
...
struct wps_pbcd_info {
	...
	char reply_buf[WPS_PBCD_MAX_BUF_SIZE];	// Buffer to get the data from wpa contrl interface
...
wps_pbcd_process_hostapd_wpasupplicant_ctrl_events(wps_pbcd_info_t *pbcd_info)
{
	...
	cprintf("\nInfo: wps_pbcd %s %d received on %s len [%d] and data [%s] \n", __func__,
	...
	if (strstr(data, WPS_EVENT_NEW_AP_SETTINGS)) {
		if (pbcd_info->get_network_settings(pbcd_info) == 0) {
			pbcd_info->apply_network_settings(&pbcd_info->bss, &pbcd_info->creds);
...
wps_pbcd_poll_button_press()
{
	...
	while (1) {
		...
		if (wps_ifnames == NULL) {
			...
			cprintf("Err: wps_pbcd %s did not find any interface to start wps "
		...
		cprintf("Info: wps_pbcd %s wps behaviour is %s timeout = %d\n", __func__,
...
wps_pbcd_fill_info_objects_list(wps_pbcd_info_list_t *list)
{
	...
	ifnames_size = strlen(lan_ifnames) + strlen(lan1_ifnames) + 2 /* space + '\0' */;
	ifnames = (char *)calloc(1, ifnames_size);
	if (!ifnames) {
		cprintf("Err: wps_pbcd %s %d failed to allocate memory of size %d\n",
			__func__, __LINE__, ifnames_size);
		return -1;
	}
	/* ifnames = lan_ifnames + space + lan1_ifnames */
	snprintf(ifnames, ifnames_size, "%s%s%s", lan_ifnames, " ", lan1_ifnames);

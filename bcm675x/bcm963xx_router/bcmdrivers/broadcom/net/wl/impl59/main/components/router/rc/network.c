 ...
 *
 * $Id: network.c 773811 2019-04-01 17:55:34Z $
 */
...
/* Function to reset interface credentials to default values for MAP onboarding */
void
reset_map(char *name)
{
	int unit = -1;
	...
	wl_ioctl(name, WLC_GET_INSTANCE, &unit, sizeof(unit));
	snprintf(tmp, sizeof(tmp), "wl%d_map", unit);
	nvram_set(tmp, "4");
	snprintf(tmp, sizeof(tmp), "wl%d_ssid", unit);
	nvram_set(tmp, "Broadcom");
	snprintf(tmp, sizeof(tmp), "wl%d_akm", unit);
	nvram_unset(tmp);
	snprintf(tmp, sizeof(tmp), "wl%d_wpa_psk", unit);
	nvram_unset(tmp);
	snprintf(tmp, sizeof(tmp), "wl%d_crypto", unit);
	nvram_unset(tmp);
}
...
start_lan(void)
{
	...
	/* If re onboarding is enabled, unset required NVRAMs*/
	if (map_reonboard) {
	...
	for (i = 0; i < MAX_NO_BRIDGE; i++) {
		...
		if (strncmp(lan_ifname, "br", 2) == 0) {
			...
			foreach(name, lan_ifnames, next) {
				...
				if (map_reonboard) {
					/* To start reonboarding, reset back all interfaces to STA.
					 * Reset back SSID and security credentials to default to
					 * ensure this STA interface does not get associated to
					 * the root AP.
					 */
					reset_map(name);
				}

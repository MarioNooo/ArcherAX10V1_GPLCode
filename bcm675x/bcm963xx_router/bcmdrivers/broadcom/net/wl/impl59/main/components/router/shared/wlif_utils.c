 ...
 *
 * $Id: wlif_utils.c 773360 2019-03-19 06:42:21Z $
 */
...
wl_wlif_scan(char *ifname, char *ssid)
{
	...
	if ((ret = wl_ioctl(ifname, WLC_SCAN, params, params_size))) {
		dprintf("Err: shared %s ifname %s scan command failed\n", __func__, ifname);
	}
	...
	char *bh_ifname, int bh_ifname_sz)
{
	...
	for (idx = 0; idx < bss_list->count; idx++) {
		...
		while (count++ < WLIF_SCAN_TRY_COUNT) {
			// Before doing scan do a wl down followed by wl up
			wl_ioctl(bss->ifname, WLC_DOWN, NULL, 0);
			wl_ioctl(bss->ifname, WLC_UP, NULL, 0);
...
static void
wl_wlif_map_get_candidate_bhsta_bsslist(char *list, wlif_bss_list_t *bss_list)
{
...
wl_wlif_map_configure_backhaul_sta_interface(wlif_bss_t *bss_in, wlif_wps_nw_creds_t *creds)
{
	...
	wl_wlif_map_get_candidate_bhsta_bsslist(list, &bss_list);

 ...
 *
 * $Id: wl_cfg80211.c 775160 2019-05-21 12:30:06Z $
 */
	...
	u8 key_idx, bool unicast, bool multicast)
{
	...
	/* Update primary key index in case of security */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0)) && !defined(WL_CLIENT_SAE)
		cfg80211_rx_mlme_mgmt(ndev, mgmt_frame, len);
#else
		wl_cfg80211_rx_mgmt(ndev, freq, mgmt_frame, len, GFP_ATOMIC);
#endif /* KERNEL_VERSION >= 3.11 && !WL_CLIENT_SAE */
	...
	s32 bssidx, bool update_ssid)
{
	...
	if (ies->wps_ie) {
		...
		/* When wps is active update wsec value with SES_OW_ENABLED to ON and upon
		 * wps completion clear SES_OW_ENABLED bit from wsec value.
		...
		if (WPS_IS_SEL_REGISTRAR_IS_TRUE(flags)) {
			wsec |= SES_OW_ENABLED;
		} else {
			wsec &= ~SES_OW_ENABLED;
		}
                     ...
                     bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	if (evt_data->ssid.SSID_len)
		memcpy(&ext_auth_param.ssid.ssid, evt_data->ssid.SSID,
			evt_data->ssid.SSID_len);
	ext_auth_param.ssid.ssid_len = evt_data->ssid.SSID_len;
		...
		struct net_device *ndev, struct cfg80211_external_auth_params *ext_auth_param)
{
	...
	memcpy(&evt_data.ssid.SSID, &ext_auth_param->ssid.ssid, ext_auth_param->ssid.ssid_len);

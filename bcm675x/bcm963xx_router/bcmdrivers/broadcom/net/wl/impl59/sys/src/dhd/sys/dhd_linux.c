 ...
 *
 * $Id: dhd_linux.c 774070 2019-04-09 08:16:27Z $
 */
...
socsram_write_to_file_no_large_buf(dhd_pub_t *pub)
{
	...
	if (dhd_bus_iovar_op(pub, "ramstart", NULL, 0, &m.bp_address,
...
void dhd_schedule_dtrace(dhd_pub_t *dhdp, uint8 *event_data, uint32 datalen)
{
	...
	dhd_deferred_work_disable(dhdp->info->dhd_deferred_wq, BIT(DHD_WQ_WORK_DTRACE));
}
...
exit:
	...
	dhd_deferred_work_enable(dhd->dhd_deferred_wq, BIT(DHD_WQ_WORK_DTRACE));
}

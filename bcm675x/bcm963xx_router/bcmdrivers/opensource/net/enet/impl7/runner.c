...
#ifdef XRDP
#if (defined(CONFIG_BCM_SPDTEST) || defined(CONFIG_BCM_SPDTEST_MODULE))
void enet_udpspdtest_test_connect(bdmf_sysb sysb, rdpa_cpu_tx_info_t *info)
{
...
inline int _rdpa_cpu_send_sysb(bdmf_sysb sysb, rdpa_cpu_tx_info_t *info)
{
    ...
    info->spdt_so_mark = 0;
#ifdef XRDP
    /* XXX: Temporary solution - like for TCP Speed test and for Speed Service */
    enet_udpspdtest_test_connect(sysb, info);
#endif
#endif

...
#define PEAK_RATE 10000000000UL
	...
	case ETHSWPOWERSETENABLE:
		if (ethswctl->type == TYPE_GET)
        {
            if (tp_enable_power_get(&ethswctl->ret_val))
                return -EFAULT;
        ...
        {
           if (tp_enable_power_set(ethswctl->val))
           {
                return -EFAULT;
           }
        ...
        case ETHSWPORTPAUSECAPABILITY:
            ...
            if (ethswctl->type == TYPE_GET)
            {
                ...
                if ((rx_enable || tx_enable) && (pfc_rx_enable || pfc_tx_enable))
                    ...
                    ethswctl->ret_val = PAUSE_FLOW_CTRL_NONE;
                else if (rx_enable || tx_enable) {
                    if (rx_enable)
                        ethswctl->ret_val = tx_enable ? PAUSE_FLOW_CTRL_BOTH : PAUSE_FLOW_CTRL_RX;
                    else
                        ethswctl->ret_val = PAUSE_FLOW_CTRL_TX;
                }
                else {
                    if (pfc_rx_enable)
                        ethswctl->ret_val = pfc_tx_enable ? PAUSE_FLOW_PFC_BOTH : PAUSE_FLOW_PFC_RX;
                    else
                        ethswctl->ret_val = PAUSE_FLOW_PFC_TX;
                }

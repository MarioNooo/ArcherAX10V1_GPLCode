...
static int tr_set_port_blog_chnl_rx(enetx_port_t *port, void *_ctx)
{
    ...
    if (port->p.bond_grp == bond_grp)
    {
        ...
        enet_set_blog_chnl_rx(port, bond_grp->blog_chnl_rx);
    }
    return 0;
}
...
#ifdef CONFIG_BLOG
        enet_set_blog_chnl_rx(port, port->n.blog_chnl);
#endif
        if (bond_grp->port_count == 0)

...
typedef struct phy_serdes_s {
    ...
    void (*sfp_plug_cb)(phy_dev_t *phy_dev, int plug);
    int flag;
...
static inline void sf2_serdes_sfp_plug_cb(phy_dev_t *phy_dev, int plug)
{
    phy_serdes_t *phy_serdes = phy_dev->priv;
    if(phy_serdes->sfp_plug_cb)
        (*phy_serdes->sfp_plug_cb)(phy_dev, plug);
}

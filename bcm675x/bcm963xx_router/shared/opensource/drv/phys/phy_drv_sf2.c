...
static int _enable_power_set = 0;

int tp_enable_power_get(int *enable)
{
	*enable = _enable_power_set;

	return 0;
}
EXPORT_SYMBOL(tp_enable_power_get);

int tp_enable_power_set(int enable)
{
	_enable_power_set = enable;

	return 0;
}
EXPORT_SYMBOL(tp_enable_power_set);

static int _tp_power_set(phy_dev_t *phy_dev, int enable)
{
	if(!_enable_power_set)
	{
		printk("[%s:%d] power set is disabled!!!\n", __FUNCTION__, __LINE__);
		return 0;
	}
	
	return mii_power_set(phy_dev, enable);
}
#endif

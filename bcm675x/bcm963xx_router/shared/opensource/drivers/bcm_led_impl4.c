...
void bcm_common_led_setAllSoftLedsOff(void)
{
    ...
    int has_power_led = 0;
    if( BpGetPowerLedGpio( &power_gpio ) == BP_SUCCESS )
        has_power_led = 1;
#endif
...
#if defined(CONFIG_TP_IMAGE)
                    if (!has_power_led || power_gpio != gpio)
#endif
...
{
    unsigned short gpio;
#if defined(CONFIG_TP_IMAGE) && (defined(_BCM94908_) || defined(CONFIG_BCM94908))
    if( BpGetPowerLedGpio( &gpio ) == BP_SUCCESS ) {
        bcm_led_driver_blink(gpio, BLINK_FREQ_NORMAL);

...
void PhyPowerSet(int onoff)
{
	...
	printk("Power %s All PHY \n", onoff ? "on" : "off" );

	/* quard lan phy */
	for (phy_id = phy_base; phy_id < phy_base + 4; phy_id ++ ) {
		 phy_miictl = bcm_ethsw_phy_read_reg(phy_id, MII_BMCR);
		 if (onoff)
		 	phy_miictl &= ~ BMCR_POWERDOWN;		/* clear the low power bit */
		 else
		 	phy_miictl |= BMCR_POWERDOWN;
		 bcm_ethsw_phy_write_reg(phy_id, MII_BMCR, phy_miictl);
	}
...
#else
/* add an implementation for other board */
void PhyPowerSet(int onoff)
{
    printk("warning: DUMMY PhyPowerSet\n");
}
#endif
#endif

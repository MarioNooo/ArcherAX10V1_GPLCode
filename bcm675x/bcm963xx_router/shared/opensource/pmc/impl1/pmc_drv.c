...
void swreg_clk_sync(void)
{
    ...
    ReadBPCMRegister(PMB_ADDR_CHIP_CLKRST, CLKRSTBPCMRegOffset(clkrst_ena_clk_31_0), &target); 
    target &= 0xff00ffff;
    WriteBPCMRegister(PMB_ADDR_CHIP_CLKRST, CLKRSTBPCMRegOffset(clkrst_ena_clk_31_0), target);

    for(i = SWR_FIRST ; i < SWR_LAST ; i++)
    {
        unsigned int val;
        val = swr_read(i, 3);
        val |= 0x0110; /* Assert PLL_EN and PLL_PHASE_EN bits */
        swrw(i, 3, val);
    }

...
typedef enum {
    ...
    rdpa_tcont_attr_chan = 5, /* chan : R : number : channel associated with TCONT  */
    rdpa_tcont_attr_orl_prty = 6, /* orl_prty : RW : enum : Priority for overall rate limiter */
...
/** Get tcont/chan attribute.
 *
 * Get channel associated with TCONT .
 * \param[in]   mo_ tcont object handle or mattr transaction handle
 * \param[out]  chan_ Attribute value
 * \return 0 or error code < 0
 * The function can be called in task context only.
 */
static inline int rdpa_tcont_chan_get(bdmf_object_handle mo_, bdmf_number *chan_)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attr_get_as_num(mo_, rdpa_tcont_attr_chan, &_nn_);
    *chan_ = (bdmf_number)_nn_;
    return _rc_;
}

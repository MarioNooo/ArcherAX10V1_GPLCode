                                ...
                                char *intfIp, int *upSpeed, int *downSpeed)
{
    ...
    {
        bcmuLog_error("Failed to get local interface that can reach %s", serverIp);
        ret = -1;
    }
    if (p)
        pclose(p);
    ...
    if (strstr(intfName, "br") != NULL)
    {
        ...
        snprintf(buf, sizeof(buf), "ping %s -c 1", serverIp);
        p = popen(buf, "r");
        if (p)
            pclose(p);

        /* Step 2. look up the arp table for the mac address of the corresponding IP */
        snprintf(cmd, sizeof(cmd), "ip neighbour | grep %s", serverIp);
        p = popen(cmd, "r");
        if ((NULL == p) || (EOF == fscanf(p, "%s %s %s %s %s", tmp, tmp, tmp, tmp, macAddr)))
        {
            bcmuLog_error("Failed to get mac address of the server from the arp table\n");
            ret = -1;
        }
        if (p)
            pclose(p);

        /* Step 3. use brctl to find the port number in the bridge that saw the mac
                   address of the server. */
        if (strlen(macAddr)) {
            snprintf(cmd, sizeof(cmd), "/bin/brctl showmacs %s | grep %s", intfName, macAddr);
            p = popen(cmd, "r");
            if ((NULL == p) || (EOF == fscanf(p, "%d", &portNumber)))
            {
                bcmuLog_error("Failed to get port number on the bridge that can reach  %s\n", serverIp);
                ret = -1;
            }
            if (p)
                pclose(p);
        ...
        if (portNumber) {
            snprintf(cmd, sizeof(cmd), "/bin/brctl showstp %s | grep '(%d)'", intfName, portNumber);
            p = popen(cmd, "r");
            if ((NULL == p) || (EOF == fscanf(p, "%s", intfName)))
            {
                bcmuLog_error("Failed to get the interface name associated to the port on the bridge\n");
                ret = -1;
            }
            if (p)
                pclose(p);
    ...
    if (strstr(intfName, "eth") != NULL) {
        *upSpeed = 1000; /* set speed to 1G if nothing works out */
        ...
        {
            bcmuLog_error("Failed to get speed for %s\n", intfName);
            ret = -1;
        }
        if (p)
            fclose(p);
                ...
                if (strstr(buf, "US Rate")) {
                    if (EOF != sscanf(buf, "US Rate = %lu bps", &tmpSpeed)) {
                        *upSpeed = tmpSpeed/1000;
                        ...
                        bcmuLog_error("Failed to get US rate for %s\n", intfName);
                        ret = -1;
                    }
                }
                if (strstr(buf, "DS Rate")) {
                    if (EOF != sscanf(buf, "DS Rate = %lu bps", &tmpSpeed)) {
                        *downSpeed = tmpSpeed/1000;
                        ...
                        bcmuLog_error("Failed to get DS rate for %s\n", intfName);
                        ret = -1;
                    }
                }
            }
